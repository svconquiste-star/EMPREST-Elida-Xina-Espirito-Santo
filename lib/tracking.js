...
class TrackingManager {
  ...
  async trackContact({ nome, telefone, email, cidade, tipoVinculo, valorDesejado }) {
    const eventId = this.gerarEventId('Contact');

    // ✅ OTIMIZAÇÃO PJ: Disparar Pixel SÓ se for PJ
    if (tipoVinculo !== 'PJ') {
      console.log('[Tracking] Lead CLT - sem disparo de Pixel Contact (estratégia PJ)');
      return { ok: true, skipped: 'CLT' };
    }
    ...
    // Disparar evento Contact com value (parâmetro padrão do Pixel)
    if (typeof window !== 'undefined' && window.fbq) {
      window.fbq('track', 'Contact', {
        currency: 'BRL',
        content_type: 'product',
        content_id: 'atendimento',
        value: valorDesejado ? parseFloat(valorDesejado) : 0,
      }, { eventID: eventId });
    }
